...
class EnsureSingleActiveSession
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($this->sessionConcurrencyManager->isCurrentSessionActive($request, $userId)) {
            // Validitas sesi web SSO tidak boleh bergantung pada status token OAuth.
            // Token yang dicabut tetap ditolak oleh Passport ketika digunakan,
            // tetapi pengguna boleh tetap login dan melakukan otorisasi ulang.
            return $next($request);
        }
